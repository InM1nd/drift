...
describe("rollInjectorDrop — dropChancePct параметризуется Уровнем угрозы (docs/11-threat-level.md)", () => {
  it("dropChancePct=0 никогда не дропает, dropChancePct=100 дропает всегда (при свободном слоте)", () => {
    for (let seed = 0; seed < 30; seed++) {
      expect(rollInjectorDrop(createRng(seed), 0, 3, 0)).toBeNull();
      expect(rollInjectorDrop(createRng(seed), 0, 3, 100)).not.toBeNull();
    }
  });
});
