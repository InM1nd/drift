...
> **ОТКЛОНЁН.** Пользователь сравнивал этот кандидат с [10 · Пиксельный трофей](10-style-pixel.md) вживую на макете и выбрал Пиксельный трофей. Реализация HUD-варианта (`HudRoomBackdrop`, `VisualStyleSwitch`, `data-visual-style="hud"` CSS-ветки и вся HUD-разметка экранов) полностью удалена из кодовой базы. Документ ниже оставлен как исторический артефакт спецификации — не источник истины для текущего кода.
...
Пустое поле визора заменяется не полноценной иллюстрацией, а полупрозрачной сенсорной реконструкцией остова. Компонент `HudRoomBackdrop` рисует окружение одним SVG-языком: перспектива переборок, технические панели, характерный объект комнаты и локальные следы Коррозии. Это сохраняет исходное ограничение HUD — только CSS/SVG, без растрового арт-пайплайна — но даёт каждому экрану собственное место.

Профили окружения:

- `compartment` — детерминированный вариант грузового отсека, лаборатории или шлюза со следами Коррозии;
- `elite` — усиленный защитный узел с янтарной геометрией;
- `boss` / `run-end` — реакторное ядро и красный аварийный регистр;
- `signal` — осциллограмма неизвестного источника;
- `shop` — консоль терминала снабжения;
- `rest` — сервисное кольцо и ремонтная крестовина;
- `map` — маршрутная схема поверх корпуса остова;
- `reward` — экран извлечения и упаковки данных.

Для одинаковых типов узлов ориентация и расположение вторичных панелей выбираются детерминированно из `node.id`, поэтому комната сохраняет вид после перезагрузки и не требует нового поля в сохранении. Фон всегда `aria-hidden`, не принимает ввод и находится ниже интерактивного слоя. В Pixel-стиле компонент полностью скрыт.

## Три плана глубины

HUD строится как один визор с тремя уровнями, а не как фон под одинаковыми карточками:

1. **Field plane** — помещение, сетка и сканирование. Самый низкий контраст, ввод отсутствует.
2. **Contact plane** — противники, узлы маршрута и кандидаты награды. Полуплотные поверхности и данные, необходимые для сравнения.
3. **Command plane** — витальные показатели, рука, Инъекторы, журнал и завершение хода. Почти непрозрачный слой с максимальной читаемостью.

На мобильном Field plane дополнительно приглушается, нижняя телеметрия театра скрывается, журнал превращается в однострочный тикер, а командная строка остаётся у нижней границы.

Цветовые регистры не взаимозаменяемы: голубой означает наведение/фокус, янтарный — Заряд/экономику/одноразовую оснастку, красный — вред и критическое состояние, зелёный — Коррозию. Обычный Корпус использует нейтральный цвет текста; голубым его полоса становится только у выбранной цели.
...
Вторичная иконографика использует тот же `IconFrame`: Корпус, Заряд, Кредиты, Угроза, тип Протокола, Модуль и Инъектор. На Протоколах форма и подпись типа работают вместе (`Атака`, `Система`, `Контур`), поэтому различие не зависит только от цвета. Эти маркеры показываются только в HUD и не меняют плотность Pixel-варианта.

## Иерархия действий и одноразовая оснастка

Протоколы и Инъекторы не должны выглядеть как один тип сущности. Протоколы остаются основной рукой крупных карточек, потому что участвуют в повторяющейся петле выбора и тратят Заряд. Инъекторы вынесены в отдельный компактный `AUX // ONE-SHOT` отсек между состоянием боя и рукой: янтарный регистр, код слота `INJ-XX`, явный маркер `1×` и горизонтальная стойка. Это подчёркивает мгновенное расходование и не смешивает инвентарь с колодой.

Тот же принцип действует в Терминале снабжения: Протоколы продаются карточной сеткой, а Модули/Инъекторы — отдельным списком «Оснастка» с собственным глифом и компактной строкой покупки.
...
| `.consumable-bay`, `.injector` | CombatScreen.css | отдельная компактная AUX-стойка; янтарный регистр, `INJ-XX`, `1×`, не использовать размеры/иерархию карточки Протокола |
...
- [ ] Каждый тип комнаты имеет узнаваемый SVG-профиль, но фон не снижает читаемость текста и полос состояния.
- [ ] Вторичные HUD-иконки скрыты в Pixel-стиле и не изменяют его карточную плотность.
